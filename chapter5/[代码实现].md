...
这里，笔者将为大家介绍，如何实现一个最简单的服务。编程语言我们使用Ruby，当然其他语言的实现非常类似。

在开始动手之前，先让我们将要做的事情划分成不同的小任务，
为了让大家对我们要做的事情有个结构性的认识，我们首先进行一个简单的任务划分，也就是列出Task，步骤如下：

<img src="images/steps_implementing_a_micro_service_800-600.png" />
...
### 使用模型存储模式(Repository Pattern)完成存储
...
在笔者的实践中，该部分实现通常使用模型存储模式(Repository Pattern)。模型存储模式是由埃里克·伊文思在他的《领域驱动设计》一书中提到，可以算是使用相当广泛的应用架构设计模式之一。
使用模型存储模式的最大优势在于：屏蔽了模型的存储以及获取的实现细节，让调用者更关注接口部分。
...
这里，Database.db.relation(:products)能够以Hash的形式读取数据库products表中的所有数据，as(:products)会帮我们把Hash映射成对应的Product对象。

当然，我们需要事先对rom-sql的relation以及数据库和model之间的映射进行配置，基本配置如下：
...
最后，我们需要通过API将服务暴露出来，API主要处理请求和响应部分。这和我们熟悉的WEB MVC框架中controller作用类似，譬如Spring MVC的controller，Rails框架中的controller，负责接收请求，调度业务处理逻辑，并返回结果。
